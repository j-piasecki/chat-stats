...
export function isSubscriberCountInChannel<T>(
  query: QueryObject<T>
  // @ts-ignore ts thinks it knows better, it doesn't
  // the types may be wrong, but thats exactly what this function checks
): query is SubscriberCountInChannel {
  return query.type === SubscriberCountInChannel.TYPE
}

export class SubscriberCountInChannel extends QueryObject<number> {
  static TYPE = 'SubscriberCountInChannel'
  readonly type: string = SubscriberCountInChannel.TYPE
  ...
  async execute(): Promise<number> {
    ...
    return await Database.getSubscriberCountInChannel(
      this._channel,
