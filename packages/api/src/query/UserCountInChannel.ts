...
export function isUserCountInChannel<T>(
  query: QueryObject<T>
  // @ts-ignore ts thinks it knows better, it doesn't
  // the types may be wrong, but thats exactly what this function checks
): query is UserCountInChannel {
  return query.type === UserCountInChannel.TYPE
}

export class UserCountInChannel extends QueryObject<number> {
  static TYPE = 'UserCountInChannel'
  readonly type: string = UserCountInChannel.TYPE
  ...
  async execute(): Promise<number> {
    ...
    return await Database.getUserCountInChannel(
      this._channel,
